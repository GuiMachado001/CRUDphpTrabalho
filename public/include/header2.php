<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Parzival's</title>

    <!-- padrão -->
    <link rel="stylesheet" href="./public/css/header/style.css">
    <link rel="stylesheet" href="./public/css/logar/login.css">

    <!-- cadastrar Usuario -->
    <link rel="stylesheet" href="./public/css/cadastrar/cadastrar-cadastrar.css">

    <!-- cadastrar Produto -->
    <link rel="stylesheet" href="./public/css/cadastroProduto/style-cadastro-produto.css">

    <!-- listar produto -->
    <link rel="stylesheet" href="./public/css/listar/listarProduto.css">
    
    <!-- editar produto -->
    <link rel="stylesheet" href="./public/css/editar/editar-produto.css">
    
    <!-- card -->
    <link rel="stylesheet" href="public/css/card/card.css">

    <!-- cadastra card -->
    <link rel="stylesheet" href="public/css/cadastrarAntoacao/cadastrar-anotacao.css">

    <!-- editar-anotacoes -->
    <link rel="stylesheet" href="public/css/editar-anotacoes/editar-anotacoes.css">

    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.6.0/css/all.min.css" integrity="sha512-Kc323vGBEqzTmouAECnVceyQqyqdsSiqLQISBL29aUW4U/M7pSPA/gEUZQqv1cwx4OnYxTxve5UMg5GT6L4JJg==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    
    
    <link rel="stylesheet" href="modal/edicao-produto/editar-produto.css">
    <script src="modal/edicao-produto/editar-produto.js"></script>
</head>
<body>
    <header>
        <i class="fa-solid fa-shop"> Parzival's Supermarket</i>
        
        <nav class="nav-bar">
            <ul class="nav-list">
                <li class="navItem active"><a href="./anotacoes.php">Anotações</a></li>
                <li class="navItem active"><a href="./areaprivada.php">lista</a></li>
                <li class="navItem active"><a href="./index.php">Sair</a></li>
            </ul>
        </nav>
    </header>